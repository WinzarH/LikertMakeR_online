...
# choose a plan (multisession is Windows-friendly)
future::plan(future::multisession)

# optional: reduce worker count if needed
...
server <- function(input, output, session) {
  ...
  validation_stats <- reactive({
    df <- syntheticData()   
    req(is.data.frame(df), nrow(df) > 0)
    
    # keep it FAST: simple summaries only
    list(
      n = nrow(df),
      p = ncol(df),
      means = vapply(df, mean, numeric(1), na.rm = TRUE),
      sds   = vapply(df, sd,   numeric(1), na.rm = TRUE),
